...
reduction is the choice the package exists to make explicit: `window`
...
- window:
  ...
  One of `"hour"`, `"halfday"`, `"day"`, `"week"`, `"month"`,
...
Whether a window is a day or coarser is decided from the bins rather
than from the window's name, so a supplied calendar that cuts inside a
day is refused for these four as well, naming the day it splits.
...
apart on the window's own calendar. A bin no unit reaches is never
built, so a month missing from the whole record would otherwise pass as
four adjacent monthly bins with one simply gone. Neither the `"hour"`
window, whose bin is the reading itself, nor a supplied calendar, which
...
deployment date carries one at each end of almost every window.
...
record's ends: on a seasonal window that is up to three months of
