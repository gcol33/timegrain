...
# averaging recovers it, so skill should rise as the window widens up to the scale of the signal.
...
test_that("keeping the extremes of a window recovers what averaging removed", {
  skip_if_not_installed("glmnet")
  # Units differ in how cold one day of each week is, and the other six days carry exactly the
  # compensating warmth, so every week has the same mean whatever the unit. The window mean is then
  # blind to the difference by construction and the window's coldest day is not.
  set.seed(74)
