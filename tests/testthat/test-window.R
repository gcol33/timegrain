window_record <- function(days = 200L, seed = 20260904L) {
  set.seed(seed)
...
window_fixture_series <- function(dir, name) {
...
window_fixture_at <- function(targets, set) {
...
test_that("the core reproduces the pure-R oracle over the window grid", {
  d <- window_record()
  at <- window_anchors()
  ...
  for (g in grid) {
    for (s in schemes) {
      ...
      x <- window_matrix(d, id, t, v, at = at, span = g[[1L]], lag = g[[2L]], bins = g[[3L]],
                         stats = s)
      o <- oracle_window_matrix(d, "id", "t", "v", at = at, span = g[[1L]], lag = g[[2L]],
                                bins = g[[3L]], stats = s)
...
test_that("a window reads only the target's own unit and only its own stretch", {
  d <- window_record(days = 60L)
  at <- data.frame(id = c("p1", "p2"),
                   when = as.POSIXct(c("2021-10-20", "2021-10-20"), tz = "UTC"),
                   stringsAsFactors = FALSE)
  x <- window_matrix(d, id, t, v, at = at, span = "10 days", stats = "mean")
...
test_that("the window digests are the ones the Python side reads", {
  dir <- fixture_dir()
  skip_if(is.null(dir), "fixtures are not in the built package")

  expected <- read.csv(file.path(dir, "window_digests.csv"), stringsAsFactors = FALSE)
  targets <- read.csv(file.path(dir, "window_targets.csv"), stringsAsFactors = FALSE)
  series <- lapply(stats::setNames(nm = unique(targets$series)), window_fixture_series, dir = dir)
  ...
  for (i in seq_len(nrow(expected))) {
    ...
    at <- window_fixture_at(targets, row$set)

    x <- window_matrix(record, id, time, value, at = at, span = row$span, lag = row$lag,
...
test_that("the window fixtures carry what the contract says they carry", {
  dir <- fixture_dir()
  skip_if(is.null(dir), "fixtures are not in the built package")
  expected <- read.csv(file.path(dir, "window_digests.csv"), stringsAsFactors = FALSE)
  targets <- read.csv(file.path(dir, "window_targets.csv"), stringsAsFactors = FALSE)
...
test_that("both guards fire with the message the fixtures pin", {
  dir <- fixture_dir()
  skip_if(is.null(dir), "fixtures are not in the built package")
  guards <- read.csv(file.path(dir, "window_guards.csv"), stringsAsFactors = FALSE)
  targets <- read.csv(file.path(dir, "window_targets.csv"), stringsAsFactors = FALSE)
  ...
  for (i in seq_len(nrow(guards))) {
    row <- guards[i, ]
    name <- unique(targets$series[targets$set == row$set])
    record <- window_fixture_series(dir, name)
    attr(record$time, "tzone") <- row$tz
    expect_error(
      window_matrix(record, id, time, value, at = window_fixture_at(targets, row$set),
...
test_that("a cell the record cannot fill names the target and the interval", {
  d <- window_record(days = 30L)
  at <- data.frame(id = "p1", when = as.POSIXct("2021-09-20", tz = "UTC"),
                   stringsAsFactors = FALSE)
  expect_error(window_matrix(d, id, t, v, at = at, span = "30 days", bins = 3L, stats = "mean"),
...
test_that("a day-level statistic is refused where a calendar day would fall in two bins", {
  ...
  at <- data.frame(id = "p1", when = as.POSIXct("2021-10-20", tz = "UTC"),
                   stringsAsFactors = FALSE)
  expect_error(window_matrix(d, id, t, v, at = at, span = "7 days", bins = 3L, stats = "cold_day"),
               "cold_day needs bins of a calendar day or coarser: target 1", fixed = TRUE)
  expect_error(window_matrix(d, id, t, v, at = at, span = "7 days", bins = 3L,
                             stats = c("cold_day", "warm_day")),
               "cold_day and warm_day need bins of a calendar day or coarser", fixed = TRUE)
  expect_silent(window_matrix(d, id, t, v, at = at, span = "7 days", bins = 3L,
                              stats = c("min", "mean", "max")))

  hour <- data.frame(id = "p1", when = as.POSIXct("2021-10-20 05:00:00", tz = "UTC"),
                     stringsAsFactors = FALSE)
  expect_error(window_matrix(d, id, t, v, at = hour, span = "7 days", bins = 7L,
                             stats = "warm_day"),
               "warm_day needs bins that open on a day boundary: target 1's window opens at ",
               fixed = TRUE)
  expect_silent(window_matrix(d, id, t, v, at = hour, span = "7 days", bins = 7L, stats = "mean"))
})
...
test_that("a window states what it was built from", {
  d <- window_record(days = 60L)
  at <- data.frame(id = c("p1", "p2"),
                   when = as.POSIXct(c("2021-10-20", "2021-10-21"), tz = "UTC"),
                   stringsAsFactors = FALSE)
  x <- window_matrix(d, id, t, v, at = at, span = "30 days", lag = "12 hours", bins = 3L,
                     stats = c("min", "mean", "max"))

  expect_s3_class(x, "timesift_matrix")
  expect_identical(attr(x, "grain"), "window")
  ...
  expect_output(print(x), "grain: window")
})

test_that("a window refuses an input it cannot answer for", {
  d <- window_record(days = 60L)
  at <- data.frame(id = "p1", when = as.POSIXct("2021-10-20", tz = "UTC"),
                   stringsAsFactors = FALSE)

  expect_error(window_matrix(d, id, t, v, at = at, span = "7 days", bins = 11L, stats = "mean"),
               "does not divide into 11 bins")
  expect_error(window_matrix(d, id, t, v, at = at, span = "7 days", bins = 0L, stats = "mean"),
               "`bins` must be a positive whole number")
  expect_error(window_matrix(d, id, t, v, at = at, span = "7 days", stats = "warmest"),
               "unknown statistic")
  expect_error(window_matrix(d, id, t, v, at = data.frame(id = "p9", when = at$when),
                             span = "7 days", stats = "mean"),
               "1 target name a unit the series does not carry, first: p9", fixed = TRUE)
  expect_error(window_matrix(d, id, t, v, at = at$id, span = "7 days", stats = "mean"),
               "`at` must be a data frame")
  expect_error(window_matrix(d, id, t, v, at = data.frame(id = "p1", when = "2021-10-20"),
                             span = "7 days", stats = "mean"),
               "must be POSIXct")
})
...
test_that("the anchors are read as a clock in the series' own calendar", {
  ...
  vienna <- window_matrix(d, id, t, v, at = at, span = "7 days", bins = 7L, stats = "cold_day")
  ...
  shifted <- data.frame(
    ...
    stringsAsFactors = FALSE)
  naive <- window_matrix(relabelled, id, t, v, at = shifted, span = "7 days", bins = 7L,
                         stats = "cold_day")
  expect_identical(as.vector(unclass(vienna)), as.vector(unclass(naive)))

  # A window is placed relative to its anchor, so an offset the same at both ends of it cancels and
  ...
  attr(d$t, "tzone") <- "UTC"
  expect_error(window_matrix(d, id, t, v, at = at, span = "7 days", bins = 7L, stats = "cold_day"),
               "open on a day boundary")
  expect_identical(as.vector(unclass(window_matrix(d, id, t, v, at = at, span = "7 days",
                                                   stats = "mean"))),
                   as.vector(unclass(window_matrix(relabelled, id, t, v, at = shifted,
                                                   span = "7 days", stats = "mean"))))
})
