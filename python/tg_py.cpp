...
std::vector<timegrain::Stat> parse_stats(const std::vector<std::string>& names) {
  std::vector<timegrain::Stat> out;
  out.reserve(names.size());
  for (const std::string& name : names) out.push_back(timegrain::stat_from_name(name));
  return out;
}
...
NB_MODULE(_core, m) {
  m.doc() = "The binning and the reduction, shared with the R package as src/tg_core.cpp.";

  nb::register_exception_translator(
      [](const std::exception_ptr& p, void*) {
        try {
          std::rethrow_exception(p);
        } catch (const timegrain::Error& e) {
  ...
  m.def("reduce",
        [](ConstI32 unit, ConstF64 value, ConstI64 when, ConstI64 local,
          ...
          timegrain::Result out = timegrain::reduce(req);
  ...
  m.def("bin_starts",
        [](ConstI64 local, const std::string& window, int year_month, int year_day) {
          std::vector<timegrain::seconds> out(local.size());
          timegrain::bin_starts(local.data(), local.size(), timegrain::window_from_name(window),
  ...
  m.def("bin_nexts",
        [](ConstI64 bins, const std::string& window, int year_month, int year_day) {
          std::vector<timegrain::seconds> out(bins.size());
          timegrain::bin_nexts(bins.data(), bins.size(), timegrain::window_from_name(window),
                               timegrain::YearStart{year_month, year_day}, out.data());
          return give(std::move(out));
        },
